...
class Khushaba2(Dataset):
    ...
    def __init__(self, base_dir, identifier, class_labels=[], **kwargs):
        """Initialize a Khushaba2 dataset without loading it.

        Args:
            base_dir (str): The path to the base directory in which the Khushaba2 dataset resides.
        ...
        # all Khushaba2 datasets have the same configuration and parameters
    ...
    def getClassTrials(self, class_name):
        ...
        for force_level in self._force_levels:
            path = self.base_dir+'{}_Force Exp/{}_{}/'.format(self.data_id, class_name, force_level)

            for i in range(1,6):
                file = path+'{}_{}_{}_t{}.mat'.format(self.data_id, class_name, force_level, str(i))

                trial = scipy.io.loadmat(file)['t{}'.format(i)][:,:7]
                trial = scipy.signal.decimate(trial,2, n=None, ftype='iir', axis=0, zero_phase=True)
